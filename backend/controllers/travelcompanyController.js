...
const SignupTravelCompany = async (req, res) => {
    try {
        ...
        if (newTravelCompany) {
            const token = genrateTokenAndSetCookie(newTravelCompany._id, res);
...
const SetPassword = async (req, res) => {
    try {
      ...
      const token = genrateTokenAndSetCookie(travelCompany._id, res);
...
const updateTravelcompany = async (req, res) => {
    ...
    try {
        ...
        res.status(200).json({ 
            ...
            token // Include token at the root level of the response
        });
